<?php
// File: chinese_traditional/localized.config.php - plus version （05.11.2017 - rev.6）
...
define("A_CONF_45", "GeoIP国家标志"); //rev.4
...
define("A_CONF_ERR_2", "Don’t forget to change remotely the name of %s directory to %s （and set its attributes to <b>777</b>）"); // %s = folder names （old|new）
...
define("A_CONFCONTENT_124", "這個 mod 是仍在發展！日程表中的字段已刻意的禁用。"); // rev.3
...
define("A_CONFCONTENT_248a", "在新的消息："); //rev.6
...
define("A_CONFCONTENT_335a", "媒體播放器預覽");
define("A_CONFCONTENT_336", "流媒體的URL路徑。");
define("A_CONFCONTENT_336a", "在上线之前，你应该测试你自己的URL。打开%s页面一些工作示例的流格式和更多的信息。");
define("A_CONFCONTENT_336b", "浏览器仍在使用一些不同的技术，因此一些格式可能无法在所有浏览器中使用不同的浏览器测试将有助于做出最好的决定和知道的限制。<br />我们将尝试改进 这个HTML5媒体播放器的跨浏览器兼容性，所以不断寻找".APP_NAME."更新。");
...
define("A_CONFCONTENT_377", "MySQL/數據庫服務器版本：");
...
define("A_CONFCONTENT_389", "這將隨機返回一個為每個用戶的動態圖像，沒有他們的電子郵件gravatar.com帳戶。（聊天嘉賓和/或未經註冊gravatar.com帳戶的用戶）。");
...
define("A_CONFCONTENT_433", "啟用在聊天中使用GeoIP國家標記。"); //rev.4
define("A_CONFCONTENT_434", "這個MOD將顯示國家名稱，國家標誌和國家代碼，基於每個用戶訪問聊天的當前/最近的位置。"); //rev.4
define("A_CONFCONTENT_435", "在所有用戶公開聊天頁面上顯示GeoIP國家標誌。"); //rev.4
define("A_CONFCONTENT_436", "即使被隱藏，高級用戶仍然可以通過標誌看到用戶的位置（為了更多的控制），通過保持使用上面啟用的標誌。"); //rev.4
define("A_CONFCONTENT_437", "GeoIP數據庫（GeoLite&trade;）保存在你的plugins/countryflags文件夾中作為GeoIP.dat和GeoIPv6.dat。這些數據庫包含基於訪問者的IP（IPv4和IPv6） MaxMind開發人員每月更新一次（每個月的第一個星期二）。IP分配可能經常從一個國家更改為另一個國家，因此，為了獲得最佳精度，您可以下載並將GeoIP.dat和GeoIPv6.dat文件替換為最新發布的版本。"); //rev.4
?>