<?php
// File: chinese_simplified/localized.config.php - plus version （05.11.2017 - rev.6）
...
define("A_CONFCONTENT_124", "这个 mod 是仍在发展！日程表中的字段已刻意的禁用。"); //rev.3
...
define("A_CONFCONTENT_335a", "媒体播放器预览");
define("A_CONFCONTENT_336", "流媒体的URL路径。");
define("A_CONFCONTENT_336a", "在上線之前，你應該測試你自己的URL。打開%s頁面一些工作示例的流格式和更多的信息。");
define("A_CONFCONTENT_336b", "瀏覽器仍在使用一些不同的技術，因此一些格式可能無法在所有瀏覽器中使用不同的瀏覽器測試將有助於做出最好的決定和知道的限制。<br />我們將嘗試改進 這個HTML5媒體播放器的跨瀏覽器兼容性，所以不斷尋找".APP_NAME."更新。");
...
define("A_CONFCONTENT_377", "MySQL/数据库服务器版本：");
...
define("A_CONFCONTENT_389", "这将随机返回一个为每个用户的动态图像，没有他们的电子邮件 gravatar.com 帐户。（聊天嘉宾和/或未经注册gravatar.com帐户的用户）。");
...
define("A_CONFCONTENT_433", "启用在聊天中使用GeoIP国家标记。"); //rev.4
define("A_CONFCONTENT_434", "这个 mod 将显示国家名称，国家标志和国家代码，基于每个用户访问聊天的当前/最近的位置。"); //rev.4
define("A_CONFCONTENT_435", "在所有用户公开聊天页面上显示GeoIP国家标志。"); //rev.4
define("A_CONFCONTENT_436", "即使被隐藏，高级用户仍然可以通过标志看到用户的位置（为了更多的控制），通过保持使用上面启用的标志。"); //rev.4
define("A_CONFCONTENT_437", "GeoIP数据库（GeoLite&trade;）保存在你的plugins/countryflags文件夹中为GeoIP.dat和GeoIPv6.dat。这些数据库包含基于访问者的IP（IPv4和IPv6） MaxMind开发人员每月更新一次（每个月的第一个星期二）。IP分配可能经常从一个国家更改为另一个国家，因此，为了获得最佳精度，您可以下载并将GeoIP.dat和GeoIPv6.dat文件替换为最新发布的版本。"); //rev.4
?>