...
Client browsers - tested with (both 32b & 64b where apply):
	...
	- Google Chrome > 78.0.x.x.x.x = H;
	- Microsoft Edge;
	- Tor Browser < 6.0.8 = H.
Server environment - tested env:
	- Apache < 2.4.41 (Unix and Windows 2k/XP/Vista/7/8/8.1/10);
	- ISS < 7;
	- php < 7.4.0;
	- MySQL < 8.0.18; (mysqli extension recommended, but mysql extension is still available for some old versions)
	- MariaDB < 5.5.54 / 10.4.9;
...
Note: development has been carried out on WampServer 3.2.0 64b, running on Windows 10 Pro Insider Preview 64b;
...
Fixes History (by the date the developing stage started for each version):
10.12.2019 - 1.98
